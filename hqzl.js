/**
 ...
 使用说明：
 1. 环境变量填写
    - hqAccount: 账号信息，格式为 手机号@密码，多账号用&分隔
    例如：
    单账号: 13800138000@password1
    多账号: 13800138000@password1&13900139000@password2
...
// 账号信息
let hqAccount = ($.isNode() ? process.env.hqAccount : $.getdata("hqAccount")) || "";
let hqAccountArr = [];
let phoneArr = [];
let pwdArr = [];
...
!(async () => {
    ...
    for (let index = 0; index < phoneArr.length; index++) {
        ...
        // 获取当前账号的手机号和密码
        let phone = phoneArr[index];
        let password = pwdArr[index];
...
async function Envs() {
    if (hqAccount) {
        if (hqAccount.indexOf("&") != -1) {
            hqAccountArr = hqAccount.split("&");
        } else if (hqAccount.indexOf("\n") != -1) {
            hqAccountArr = hqAccount.split("\n");
        } else {
            hqAccountArr.push(hqAccount);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 hqAccount`)
        return false;
    }

    // 解析账号和密码
    for (let account of hqAccountArr) {
        if (account.indexOf("@") != -1) {
            let [phone, pwd] = account.split("@");
            if (phone && pwd) {
                phoneArr.push(phone.trim());
                pwdArr.push(pwd.trim());
            }
        }
    }

    if (phoneArr.length === 0 || pwdArr.length === 0) {
        log(`\n 【${$.name}】：账号格式错误，请检查格式是否为 手机号@密码`)
        return false;
    }

    if (phoneArr.length !== pwdArr.length) {
        log(`\n 【${$.name}】：账号密码数量不匹配！`)
        return false;
    }
...
function checkResponse(result) {
    return result && result.code === '000000';  // 使用字符串比较
}
